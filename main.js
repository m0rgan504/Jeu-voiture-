// ============================
// MAIN.JS — Point d'entrée (corrigé)
// ============================
...
function checkSeasonEnd() {
  ...
  if (doneRaces >= totalRaces && totalRaces > 0) {
    setTimeout(() => showSeasonEndModal(), 1000);
  }
}
...
function showSeasonEndModal() {
  ...
  UI.openModal("🏆 FIN DE SAISON " + s.season,
    `<div style="text-align:center;padding:16px 0">
      ...
      <div class="warn-box">
        💸 Salaires à venir : <strong>${formatMoney(wages)}/mois</strong>
      </div>
    </div>`,
    `<button class="btn-primary" onclick="nextSeason()">→ Saison ${s.season + 1}</button>`
  );
}
...
window.addEventListener("DOMContentLoaded", () => {
  ...
  // MENU PRINCIPAL
  document.getElementById("btnNewGame").addEventListener("click", () => showScreen("create"));

  document.getElementById("btnContinue").addEventListener("click", () => {
    if (GAME.load()) {
      launchGame();
    } else {
      UI.toast("Aucune sauvegarde trouvée.", "error");
    }
  });

  document.getElementById("btnCredits").addEventListener("click", () => {
    UI.openModal("CRÉDITS",
      `<p style="line-height:2.2;color:var(--text2)">
        <strong style="color:var(--text);font-size:1.1rem">APEX MOTORSPORT</strong><br>
        ...
        <span style="color:var(--text3);font-size:0.8rem">Version 1.0.0 — Sauvegarde automatique</span>
      </p>`,
  ...
  // BOUTONS MARCHÉ
  document.getElementById("btnBuyCar").addEventListener("click", () => UI.openCarMarket());
  document.getElementById("btnHireDriver").addEventListener("click", () => UI.openDriverMarket());
  document.getElementById("btnHireMechanic").addEventListener("click", () => UI.openMechanicMarket());
